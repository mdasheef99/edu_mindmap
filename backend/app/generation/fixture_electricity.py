...
class ElectricityFixtureProvider:
    ...
    _nodes: dict[str, tuple[str, str]] = {
        "overview": (
            ...
            "Electricity studies how charges move through circuits and how that movement "
            "powers devices.",
        ),
        "electric-current": (
            "Electric current",
        ...
        "parallel-combination": (
            "Parallel combination",
            "In a parallel circuit, components have separate paths and the equivalent "
    ...
    def child_for_choice(self, *, source_key: str, selected_option_text: str) -> GeneratedNode:
        ...
        kind: Literal["generated", "fallback"] = (
            "generated" if source_key in self._nodes else "fallback"
        )
        return self._node(
            next_key,
            kind=kind,
