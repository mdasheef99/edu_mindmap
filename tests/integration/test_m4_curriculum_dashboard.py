...
def test_curriculum_endpoints_return_electricity_launch_path():
    ...
    subjects = client.get(
        f"/v1/student/curriculum/subjects?class_id={seed.class_level_id}&exam_id={seed.exam_id}"
    )
    chapters = client.get(
        "/v1/student/curriculum/chapters"
        f"?class_id={seed.class_level_id}&exam_id={seed.exam_id}&subject_id={seed.subject_id}"
    )
    concept_entries = client.get(f"/v1/student/chapters/{seed.chapter_id}/concept-entries")
