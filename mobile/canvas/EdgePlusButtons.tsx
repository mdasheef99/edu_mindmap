...
export function EdgePlusButtons({
  ...
  function renderButton(side: 'left' | 'right', style: object) {
    const isActiveSide = requestState.side === side;
    const isRetry = requestState.phase === 'failed' && isActiveSide;
    const label = isLoading && isActiveSide
      ? 'Loading questions'
      : isRetry
        ? `Retry loading questions from ${side} edge`
        : `Explore from ${side} edge`;
    return (
      <Animated.View testID={`edge-plus-${side}-wrapper`} style={style}>
        <Pressable
          ...
          disabled={disabled || isLoading || (requestState.phase === 'failed' && !isActiveSide)}
        ...
        {isActiveSide && requestState.phase !== 'idle' ? (
          <View pointerEvents="none" style={[styles.feedback, side === 'left' ? styles.feedbackLeft : styles.feedbackRight]}>
