...
def main() -> None:
    ...
    token = _signup(supabase_url, _required("SUPABASE_ANON_KEY"))
    headers = _auth(token)

    runtime = build_postgres_runtime(database_url=database_url, supabase_url=supabase_url)
    ...
    restarted = build_postgres_runtime(database_url=database_url, supabase_url=supabase_url)
