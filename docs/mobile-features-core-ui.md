...
| Recommended chapters | Dashboard → "For You" section | Advanced | AI-suggested based on recent exploration and study activity | AI service, analytics |
...
| Display short node label | Within node body | Basic | Compact text for quick recognition | None |
| Change node color | Node toolbar → Color picker | Basic | 8-color palette (reduced from 16) | None |
| Copy/duplicate node | Node toolbar → "More" menu | Basic | General utility action; not the primary branching model | None |
| Delete node | Long-press → "Delete" or swipe left | Basic | Confirmation dialog | None |
| Create manual connection to other node | Node toolbar → "Connect" → tap target | Basic | Visual preview for reference-style link (separate from AI-generated path edges) | Target node |
...
### 2.2 AI Node (Dynamic Content Node)
...
| Edge + branch trigger | Left and right vertical node edges → "+" button | Basic | Opens next-question options from either side of the node | AI service |
| Delete node | Long-press → "Delete" | Basic | Confirmation required; deleting a node in an AI-generated exploration path also deletes its descendant path nodes | None |
| Create manual connection to other node | Node toolbar → "Connect" | Basic | Reference-style connection; separate from AI-generated path edges | Target node |
| Auto-create path edge on branch creation | Automatic | Basic | New child AI nodes connect to parent via AI-generated exploration-path edge | Canvas |


### 2.3 Video Node (YouTube)
...
| View node label | Properties panel → "Name" | Advanced | Shows node prompt/title for reference | Node selection |
...
| FAB expansion | Tap FAB | Basic | Reveals available node type options | FAB |
...
| Connection context menu | Long-press on connection line | Advanced | View connection details; manual links may be removed, while AI-generated path edges follow path-structure deletion rules | Connection |
