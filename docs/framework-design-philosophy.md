...
## Purpose
This document defines the **design philosophy** for a **path-based conceptual exploration and teacher-support system**: how the system supports **path-based conceptual exploration**, where **teacher-support interpretation** fits, what we are (and are not) claiming, and how those claims translate into product + data decisions.
...
- an **internal analytic and teacher-support model** for interpreting conceptual exploration paths and possible follow-up areas, and
- a claim of **provisional sufficiency**: these categories may be adequate for teacher-support interpretation and internal analytic research given current evidence, with sufficiency treated as an open empirical question under ongoing test.
...
- Post-hoc classification of questions as **multi-dimensional engagement vectors**—independent scores (0.0–1.0) across all 8 analytic dimensions—yields a richer “learning fingerprint” than single-category assignment, because a single question can genuinely engage multiple dimensions simultaneously.
...
1. **Cumulative State** — running sum of dimensional vectors across all selected questions, normalised by question count. This is the primary input for identifying lower-engagement or higher-priority follow-up areas: `normalised_cumulative[d] = Σ scores[d] / N`.
2. **Trajectory Shape** — how the cumulative profile evolves over the session. Early exploration may cluster in one region of the dimensional space; later exploration may diversify. The shape (not just the endpoint) is interpretively informative for teachers and future reinforcement.
...
## 2) Canonical category set (internal analytic lens)
These categories are used as an internal analytic lens because they provide principled dimensions for analyzing conceptual engagement, derived from a serious philosophical tradition. Any given concept engages these dimensions unevenly; subject-specific weights encode which dimensions are structurally available and comparatively important for interpretation. Low coverage in high-weight dimensions should be treated as a **possible area for follow-up**, not as proof of misunderstanding.
...
Subjects differ because different subjects systematically produce concepts with different structural profiles. Weights encode the typical structural availability and relative interpretive priority of each dimension across the concept types characteristic of a given subject. A high weight means the dimension tends to be prominently and meaningfully available in that subject's concepts. A low weight means the dimension tends to be thinner or less central for follow-up interpretation—present in the categorical space but not meaningfully instantiated in most of that subject's concept types. Weights are implemented via the weight matrices defined in `docs/subject-weighting-specification.md`.

**Interpretation rule**: weights prioritize teacher attention and research analysis; they should not be read as proof that a learner has or lacks understanding.

**Rule**: adapt interpretation and prioritization via weights; do not delete dimensions.

---

## 3.5) How weights flow through the pipeline (interpretive lens, not a generation constraint)
Subject weights are an **interpretive lens** used downstream; they must not be used to "steer" Stage 1 generation.
...
- **Scoring / analytics**: compute **weighted coverage** and **follow-up priority signals** using subject weights.
- **Presentation**:
  - **Student**: category-neutral language only (no labels, no axis names, no dimension counts).
  - **Teacher/Admin**: may see weighted patterns, possible follow-up areas, and category-level breakdown.
- **Intervention**: teacher recommendations may prioritize **high-weight low-coverage areas** (subject-specific), while student-facing prompts remain category-neutral and lower-certainty.
...
- **Periodic re-randomization / randomized probes**: even for exploitation-mode students, periodically include small amounts of safely-randomized or organic content to detect preference drift and evaluate whether personalization is steering behavior.
...
We define **structural coverage of conceptual engagement** operationally as:
...
**Working hypothesis**: higher weighted coverage (especially in high-weight dimensions for that subject) may correlate with better downstream learning outcomes. This must be tested and recalibrated.
